...
library InvariantHelpers {
    ...
    function getUtilizationRate(
        ...
        utilizationRate = ionPool.debt().radDivDown(
    ...
    function getIlkSpecificUtilizationRate(
        ...
        utilizationRate = ionPool.totalSupply() == 0 // Prevent division by 0
            ? 0
            : (ionPool.totalNormalizedDebt(ilkIndex) * ionPool.rate(ilkIndex)) /
                (
                    ionPool.totalSupply().wadMulDown(
                        distributionFactors[ilkIndex].scaleUpToWad(4)
                    )
                );
    }
}
